# ANE (CoreML) backend — original pre-implementation plan (superseded)

> **Historical record.** This is the pre-implementation design for the native ANE
> backend. The shipped design differs; the authoritative user documentation is
> [`docs/ane-backend.md`](../ane-backend.md). Known divergences: the shipped bucket
> ladder is `[512, 768, 1536, 3000]`, not `[384, 768, 1536, 3000]` (384 was a fill
> floor, not a bucket), and `EnumeratedShapes` was rejected in favour of per-bucket
> FIXED-shape `.mlpackage`s.
