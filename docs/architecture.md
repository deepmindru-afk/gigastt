...
Embed inference in any Rust project with `gigastt-core = "2.16"`. For a lean embedded build, disable defaults (`default-features = false`) to drop `tokio` / `reqwest` / `symphonia` / polyvoice; opt capabilities back in via the `net`, `async-pool`, `file-decode`, and `diarization` features.
