...
линия или захват звука в браузере поступают как сырой PCM16, а
инкрементальный текст появляется по ходу речи. Первый partial обычно
**меньше двух секунд** (TTFP p50 ~0.82 с far-field / ~1.65 с crowd по
протоколу 1.0) — не sub-200 ms. WER стрима примерно на **11–15 п.п. хуже**
REST batch на тех же файлах (обрезанные / потерянные слова). Для файлов,
...
Вы строите живую интеграцию — субтитры для встреч, голосового бота на
линии или диктофон, который показывает partial'ы во время речи. Аудио идёт
непрерывно; пользователи ждут растущий транскрипт, чистую финализацию
фразы и хвост после `stop`. Не обещайте batch-WER на этом пути. Некоторые сессии длятся часами, в некоторых конфигурациях
...
- WebSocket-стек для вашего языка: `pip install websockets` для рецептов на
  Python, Node.js ≥ 20 для JavaScript (глобальный `WebSocket` на Node ≥ 22;
  пакет `ws` — запасной путь на 20–21), Go 1.23+ для SDK.
...
средний softmax по токенам слова — char-кусочки на `rnnt` / `ml_ctc*`,
BPE на `e2e_rnnt`). Это среднее softmax-оценок, **не
...
стоит около **20 МБ resident** (215 МБ энкодера mapped и общий; `ps` RSS
завышает). Сервер по-прежнему ограничивает пул по доступной RAM при
загрузке:
...
**Node.js** — глобальный `WebSocket` на Node ≥ 22 (без зависимостей);
Node 20–21 нужен пакет `ws` (как у `@gigastt/client`). Тот же
контракт WAV-файла:
