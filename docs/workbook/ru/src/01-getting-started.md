...
- **Диск:** ~250 МБ для дефолтной «лёгкой» INT8-установки; ~1,5 ГБ, если идти
  путём `--fp32` (загрузка FP32 плюс квантизация на устройстве).
...
и [Что стоит первый запуск](#что-стоит-первый-запуск).
...
# Скачать модель (по умолчанию ~225 МБ предквантизованного INT8 —
# см. «Что стоит первый запуск» ниже):
gigastt download
...
~225 МБ из закреплённого GitHub Release (без ~844 МБ FP32-загрузки и без
...
контейнер будет заново скачивать ~225 МБ при каждом пересоздании. При первом
старте контейнер скачивает модель — порт поднимается сразу,
...
| `ml_ctc` | ru/en/kk/ky/uz | «Голый» lowercase, без восстановления | Смешанная русско-английская (или kk/ky/uz) речь; ~1,5× RTF vs `rnnt` (ready RSS ≈ `rnnt`) |
...
## Что стоит первый запуск

Самый первый `gigastt download` (или первый `gigastt serve`, который
автоматически скачивает отсутствующую модель) скачивает **предквантизованный
INT8-бандл ~225 МБ** из закреплённого GitHub Release: с проверкой SHA-256,
через промежуточный `.partial` и атомарное переименование. Без FP32-загрузки,
без локальной квантизации, без `protoc`.

Обратите внимание: файлы берутся с `github.com`, а не с `huggingface.co` —
полезно, когда один из двух хостов заблокирован.

Три рычага меняют цену:

- `gigastt download --fp32` — старый путь: опциональный FP32 (~844 МБ) ONNX-файлов с
  HuggingFace, затем разовая ~2-минутная INT8-квантизация, дающая тот же
  энкодер ~225 МБ, который движок реально загружает. Требует `protoc`.
  Нужен, когда FP32-энкодер требуется для отладки или офлайн-квантизации.
- `--fp32 --skip-quantize` (или `GIGASTT_SKIP_QUANTIZE=1` у `serve`) —
  оставить FP32-энкодер и пропустить квантизацию. Движок тогда
...
- **Первый `serve` «висит»** — это разовая загрузка модели, а не зависание:
  `/health` в это время возвращает `{"model":"loading"}`. Подготовьте модель
  заранее командой `gigastt download` и настройте клиентов на ожидание `/ready`.
