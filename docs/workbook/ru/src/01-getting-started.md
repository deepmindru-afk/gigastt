...
- **Аудиофайл для транскрибации** — WAV, M4A, MP3, OGG/Vorbis, OGG/Opus
  (`.opus`), WebM/Opus или FLAC. Подойдёт любая короткая запись русской речи.
...
```sh
# Определить тег последнего релиза (или задайте TAG=v2.18.0 вручную):
...
$TAG = $rel.tag_name          # например v2.18.0
$VER = $TAG.TrimStart('v')
...
Runtime — **только INT8**: нет пути загрузки/инференса FP32.

Если пропустить ручной `download`, первый `serve` скачает модель сам. Порт
поднимается сразу; `/health` отвечает `200` с `"model":"loading"`, а `/ready`
возвращает `503 {"reason":"initializing"}`, пока модель не готова — клиенты
должны ждать `/ready`, а не сам факт запущенного процесса.
