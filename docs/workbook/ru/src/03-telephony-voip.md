...
в WAV, G.722 в WAV (теги `0x0064` / `0x0065` / `0x028F`), GSM 06.10 в WAV
(wav49 / тег `0x0031`), OGG/Opus, а также сырые потоки без заголовков —
через явную подсказку кодека. Эта глава ведёт от «папки со странными
файлами» к рабочим транскриптам, включая разделение спикеров по каналам
для стереозаписей.
...
- `ffprobe`/`ffmpeg` — только для определения формата и запасного
  варианта с конвертацией G.729. Для поддерживаемых форматов не нужны.
...
| `codec_name=adpcm_g722`, тег `[0x0064]`, `[0x0065]` или `[0x028f]` | G.722 в WAV | отправлять как есть |
| `codec_name=gsm_ms` | wav49 (GSM 06.10 в WAV) | отправлять как есть |
...
Три тега G.722 — один и тот же кодек от разных писалок: `0x0064` приходит
из выгрузок в стиле SBC/Asterisk, `0x0065` — имя mmreg, `0x028F` — из
инструментов на базе ffmpeg. gigastt принимает все три.
...
- **`wav49`** — GSM 06.10 в WAV-контейнере (`codec_name=gsm_ms`, тег
  `0x0031`). Отправлять как есть (8 кГц моно, ресемплинг в 16 кГц).
  33-байтовые toast / MSN блоки переменной длины не декодируются —
  такие файлы сначала конвертируем ffmpeg:
...
WAV-контейнере — `codec_name=adpcm_g722`, тег `0x0064`, `0x0065` или `0x028F`.
...
| WAV с G.722 ADPCM (теги `0x0064`, `0x0065`, `0x028F`) | да | нет | декодируется в родные 16 кГц |
...
| wav49 (GSM 06.10 в WAV) | да | нет | 8 кГц моно, ресемплинг в 16 кГц |
...
ошибку ниже); пути, держащие весь буфер целиком (диаризация,
`channels=split` и кодеки сырой телефонии), всё равно ограничены
фиксированным потолком ~30 минут.
...
- **`422` — «Check audio format»** (`invalid_audio` / `transcription_error`).
  ...
  причины: сырой поток отправлен без `codec=`; файл G.729; обрезанная/битая
  загрузка. Определите файл шагом 0.
...
- **«Audio file too long»** (`audio_too_long`, HTTP 413). По умолчанию лимита
  ...
  потолок ~30 минут на пути, держащем весь буфер целиком, — диаризация,
  `channels=split` или кодеки сырой телефонии. На основном пути делить
...
- [crates/gigastt-core/src/inference/audio/](https://github.com/ekhodzitsky/gigastt/tree/main/crates/gigastt-core/src/inference/audio) —
  внутреннее устройство декодеров (WAVE-ingest через ryf, путь Opus, детект
  dual-mono).
