...
- Папка с аудиофайлами: WAV, MP3, M4A, OGG, FLAC, WebM (вложенные папки
...
- `--pool-size` — параллельные воркеры. Дополнительный слот ~20 МБ
  resident (mapped энкодер). Цифры:
...
Прежде чем поднимать `--pool-size`, прикиньте память: ~46 / ~66 МБ
resident при пуле 1 / 2 (~277 / ~510 `ps` RSS); слот ~20 МБ. Завышенный
запрос **урезается с предупреждением** (бюджет `2 × размер файла`).
`--encoder-intra-threads` не задавайте. Цифры:
[docs/benchmarks.md](https://github.com/ekhodzitsky/gigastt/blob/main/docs/benchmarks.md).
...
- **Ожидание параллелизма, которого не получили.** `--pool-size 16` на
  8 ГБ часто урезается (предупреждение при загрузке). Смотрите лог. RAM:
  [docs/benchmarks.md](https://github.com/ekhodzitsky/gigastt/blob/main/docs/benchmarks.md).
