...
- `--pool-size` — параллельные воркеры; энкодер mapped и общий, поэтому
  дополнительный слот стоит ~20 МБ resident (не вторая копия 215 МБ).
  Масштабируйтесь по ядрам и консервативному RAM-капу при загрузке (см.
...
требует `--sample-rate`. Всё остальное — AMR, видео MP4, битый файл —
...
- Дополнительные воркеры делят mapped INT8-энкодер: **~20 МБ resident на
  дополнительный слот**. Пул по умолчанию из 2 ≈ 66 МБ resident / ~510 МБ
  `ps` RSS.
...
- Runtime — **только INT8** (~215 МБ энкодера на диске после `gigastt download`).
  FP32-инференса для batch-воркеров нет.
...
- **Ожидание параллелизма, которого не получили.** `--pool-size 16` на
  ...
  Проверяйте стартовый лог и считайте ~20 МБ resident на дополнительный
  слот. Ограничение при загрузке по-прежнему закладывает консервативные
  `2 × размер-файла-энкодера` на слот, поэтому `--pool-size 16` на 8 ГБ
  всё ещё может урезаться, даже если resident влез бы.
- **`invalid audio` / 422 на неподдерживаемом контейнере.** AMR, видео
  MP4 или битая выгрузка не декодируются. Сначала сконвертируйте
  (`ffmpeg -i in.amr -ar 16000 -ac 1 out.wav`) или, для raw-телефонии,
  объявите `--codec` + `--sample-rate`. WebM/Opus из браузера конвертировать
  не нужно. Файл `.opus` декодируем, но невидим для batch/watch —
  переименуйте в `.ogg`.
