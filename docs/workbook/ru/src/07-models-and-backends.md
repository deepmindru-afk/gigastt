...
INT8-энкодер **memory-mapped и общий** для слотов пула. Дополнительные слоты
добавляют состояние decoder/joiner и арены ORT — не вторую копию энкодера.

Измерено на Apple M1 Pro, CPU INT8, release (resident = macOS `footprint`
dirty+compressed; `ps` RSS считает общую mapping — метод в
[docs/benchmarks.md](https://github.com/ekhodzitsky/gigastt/blob/main/docs/benchmarks.md)):

| Голова (как загружена) | Файл энкодера | Resident пул 1 / 2 | `ps` RSS пул 1 / 2 |
|---|---|---|---|
| `rnnt` / `e2e_rnnt` INT8 | ~215 МБ | ~46 / ~66 МБ | ~277 / ~510 МБ |
| `ml_ctc` INT8 | ~225 МБ | ~28 МБ / — | ~261 МиБ / — |
| `ml_ctc_large` INT8 | ~592 МБ | — | — |

Тире значит **не переизмерено** после mmap — не выдумывайте `2 × размер файла`.
Ограничение RAM при загрузке по-прежнему закладывает консервативные
`2 × размер-файла-энкодера` на слот, поэтому завышенный `--pool-size` может
урезаться, даже если resident влез бы.

**Edge / мало RAM:** предпочитайте `--pool-size 1` (~46 МБ resident / ~277 МБ
`ps` RSS для `rnnt`). Так же
...
ядрах. Дефолт 2 оставляйте, когда нужны две параллельные сессии.
...
Эмпирическое правило: считайте **resident** (~46 МБ + ~20 МБ на дополнительный
слот `rnnt`). На машине с 4 ГБ пул 2 для `rnnt` — не проблема RAM; на edge
оставляйте пул 1 ради **RTF** (и пока `ml_ctc_large` не измерен). Лимиты
cgroup/пода ставьте выше `ps` RSS — тот же вывод, что и в пункте про OOM в
...
- **OOM после переключения на `ml_ctc_large`.** Resident после mmap
  **не измерен** (файл энкодера ~592 МБ; не выдумывайте `2 × размер файла`).
  Пока нет замера — `--pool-size 1`. Ограничение при загрузке по-прежнему
  закладывает `2 × размер-файла-энкодера`, поэтому пул может урезаться, даже
  если resident влез бы. Держите `--pool-min-size 1`, чтобы тесный хост
  загружался деградированно.
