...
INT8-энкодер **memory-mapped и общий**. Дополнительные слоты добавляют
состояние decoder/joiner и арены ORT — не вторую копию энкодера.
RAM процесса: ~46 / ~66 МБ resident при пуле 1 / 2 (~277 / ~510 МБ `ps`
RSS); слот сверх первого ~20 МБ. `--pool-size 1` — выбор ради RTF / edge,
а не из-за нехватки RAM. При загрузке по-прежнему закладывается
`2 × размер-файла-энкодера` на слот. Цифры:
[docs/benchmarks.md](https://github.com/ekhodzitsky/gigastt/blob/main/docs/benchmarks.md).
