...
| Память | Модель и пул живут внутри вашего приложения (~46 МБ resident при пуле 1 / ~66 МБ при пуле 2; `ps` RSS ~277 / ~510 МБ считает mapped энкодер) | Модель живёт в отдельном серверном процессе |
...
- **Директория с моделью** — все рецепты ниже её предполагают. Один раз
  скачайте преквантизованный INT8-бандл (~225 МБ lean-установка, без
  скачивания FP32 и без квантизации на устройстве):
...
1. **Добавьте пакет.** Удалённый SwiftPM
   (`https://github.com/ekhodzitsky/gigastt-swift`) — **упаковка в работе**,
   не версионированный релиз. Для разработки подключайте in-tree
   `packaging/swift` как локальную path-зависимость. Xcode → File → Add
   Package Dependencies… — только после того, как зеркало опубликует
   тегированный xcframework. SwiftPM требует `Package.swift` в корне
   репозитория, поэтому поддиректорию монорепо по URL не подключить.
...
3. **Загрузите движок и транскрибируйте:**
   ...
   // poolSize: 1 — дефолт на устройстве (~46 МБ resident / ~277 МБ ps RSS).
...
6. **Версионное рукопожатие по HTTP.** `GET /health` возвращает 200 в *обеих*
   фазах — `{"status":"ok","model":"loading","version":"2.18.0"}` во время
   bootstrap, затем `{"status":"ok","model":"gigaam-v3-rnnt","variant":"rnnt","version":"2.18.0","punctuation":true,"itn":true}`.
...
8. **Корректная остановка.** Пошлите SIGTERM: сервер дожидается завершения
   ...
   приложения — осиротевший sidecar держит ~510 МБ `ps` RSS (при `--pool-size 2`) и порт.
...
- **Память**: resident в ожидаемых пределах — ~46 МБ при пуле 1 / ~66 МБ при
  пуле 2 (`ps` RSS ~277 / ~510 МБ считает mapped энкодер);
