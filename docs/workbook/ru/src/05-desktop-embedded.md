...
6. **Версионное рукопожатие по HTTP.** `GET /health` возвращает 200 в *обеих*
   фазах — `{"status":"ok","model":"loading","version":"2.15.0"}` во время
   bootstrap, затем `{"status":"ok","model":"gigaam-v3-rnnt","variant":"rnnt","version":"2.15.0","punctuation":true,"itn":true}`.
