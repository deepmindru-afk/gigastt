...
docker run -d --name gigastt \
  ...
  ghcr.io/ekhodzitsky/gigastt:2.17.0
```
...
- **CUDA**: `docker run --gpus all -p 127.0.0.1:9876:9876
  ghcr.io/ekhodzitsky/gigastt:2.17.0-cuda` (requires the NVIDIA Container
...
```sh
gh release download v2.17.0 -R ekhodzitsky/gigastt \
    -p 'gigastt-2.17.0-offline-x86_64-unknown-linux-gnu.tar.gz' \
    -p 'gigastt-2.17.0-offline-x86_64-unknown-linux-gnu.tar.gz.sha256' \
    -p 'gigastt-2.17.0-offline-x86_64-unknown-linux-gnu.tar.gz.minisig'
sha256sum -c gigastt-2.17.0-offline-x86_64-unknown-linux-gnu.tar.gz.sha256
minisign -Vm gigastt-2.17.0-offline-x86_64-unknown-linux-gnu.tar.gz -p gigastt.pub
gh attestation verify gigastt-2.17.0-offline-x86_64-unknown-linux-gnu.tar.gz \
...
```sh
tar xf gigastt-2.17.0-offline-x86_64-unknown-linux-gnu.tar.gz
cd gigastt-2.17.0-offline
...
```sh
sudo dpkg -i gigastt_2.17.0_amd64.deb gigastt-model-int8_2.17.0_all.deb
sudo systemctl enable --now gigastt
```
...
```sh
minisign -Vm gigastt-2.17.0-x86_64-unknown-linux-gnu.tar.gz -p gigastt.pub
gh attestation verify gigastt-2.17.0-x86_64-unknown-linux-gnu.tar.gz \
    --repo ekhodzitsky/gigastt
```
...
```sh
TAG=$(gh api repos/ekhodzitsky/gigastt/releases/latest -q .tag_name)   # e.g. v2.17.0
VER=${TAG#v}
```

Docker (upgrade to the pin you chose — here `2.17.0`):

```sh
docker pull ghcr.io/ekhodzitsky/gigastt:2.17.0
...
```sh
sudo dpkg -i gigastt_2.17.0_amd64.deb
