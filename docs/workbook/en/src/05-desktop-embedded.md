...
6. **Version handshake over HTTP.** `GET /health` returns 200 in *both*
   phases — `{"status":"ok","model":"loading","version":"2.18.0"}` during
   bootstrap, then `{"status":"ok","model":"gigaam-v3-rnnt","variant":"rnnt","version":"2.18.0","punctuation":true,"itn":true}`.
