        ...
        if !is_valid {
            ...
            if parent_end.is_none() {
                for ancestor in self.stack.iter().rev().skip(1) {
                    // The candidate would be a *child* of the ancestor, so it is the child's depth
                    // that must satisfy the schema's minimum, not the ancestor's own. Passing the
                    // ancestor's depth understates it by one and rejects every element whose
                    // minimum depth equals its parent's depth plus one — which is every one of
                    // them. That is why an unknown-size Cluster could never be terminated by the
                    // Cluster that follows it: Cluster has a minimum depth of 1 and Segment sits
                    // at depth 0, so `1 <= 0` failed and a live WebM stream (what MediaRecorder
                    // writes) errored with `UnexpectedElement`.
