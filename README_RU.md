...
**WER** чистая 3.55% / far-field 4.08% / телефон 18.50% / YouTube 10.91%  ·  **held-out** CV **2.63%** (лучше Vosk+FW) · FLEURS 5.26% (лидер FW 3.84) · RuLS **4.21%** (лучше Vosk+FW) · SOVA device: Vosk впереди  · ToneWebinars: лидер FW 8.33 (gigastt 13.0)  ·  **RTF** ~0.10  ·  **Модель** ~225 МБ INT8  ·  **Холодный старт** 0.94 с  ·  **RAM** ~750 МБ одна сессия / 1.3 ГБ pool-2  ·  **Стриминг** первый partial ~0.78 с
...
|---|--:|--:|--:|--:|--:|--:|--:|--:|---|---|
| **gigastt** (GigaAM v3 `rnnt`) | 3.55 | **4.08** | **18.50** | **10.91** | 0.10 | ~225 МБ | 1.3 ГБ / ~750 МБ | **0.94 с** | **Да** — инкр. WS | **Да** |
...
Условия: Apple M1, CPU EP, INT8/greedy, 1000 сэмплов на домен (чистая речь 992; turbo — срез 300), 95% bootstrap CI. Чистая речь 3.55 (2.9–4.2) пересекается с Vosk 0.54 2.97 (2.4–3.6) — статистическая ничья; победы на far-field / телефоне / YouTube CI-раздельны. RTF &gt; 1.0 = медленнее реального времени на CPU. RAM gigastt — footprint процесса на /ready, замер на Apple M1 (INT8): ~1.3 ГБ при дефолтном `--pool-size 2`, ~750 МБ при `--pool-size 1`. «—» = не замерялось. Полная методология и оговорки: [Benchmarks](docs/benchmarks.md).
...
- **По пиковой RAM при дефолтном `--pool-size 2` (~1.3 ГБ) — самый тяжёлый вариант**: проигрывает Vosk 0.54 (560 МБ) и T-one greedy (672 МБ), а даже одна сессия (~750 МБ, `--pool-size 1`) чуть выше Vosk. Для экономичных деплоев ставьте `--pool-size 1`.
...
Rust **1.88+**, `protoc` в `PATH`. macOS 14+ (Apple Silicon, CoreML) или Linux x86_64 (опц. NVIDIA CUDA 12+). ~1.5 ГБ диска, ~1.3 ГБ RAM при дефолтном `--pool-size 2` (~750 МБ на одну сессию). Крейт `gigastt-core` без серверных зависимостей: `gigastt-core = "2.16"`.
